make.temporal.plot <- function(mutation.rates, mutation.set)
{
  ...
  temporal.plot <- ggplot(final.data, aes(x=month, y=rate, color=mutation, group=mutation)) + 
    ...
    labs(x="Month",y="Mutation rate (%)") + theme_bw() +
...
make.correlation.plot <- function(mutation.rates,corr.plot.opt)
{
  mut.names <- mutation.rates$Mutation
  if("Use clades" %in% corr.plot.opt)
  {
    mutation.rates <- mutation.rates[,grepl("_relRate",names(mutation.rates))]
    names(mutation.rates) <- gsub("_relRate","",names(mutation.rates))
  }
  else
    mutation.rates <- mutation.rates[,!grepl("Rate|Mutation",names(mutation.rates))]
  mutation.rates <- mutation.rates[,colSums(mutation.rates)>0]
  temporal.pair.rates <- data.frame(time=names(mutation.rates),mut1=as.numeric(mutation.rates[1,]),mut2=as.numeric(mutation.rates[2,]))
...
names(metadata) <- c("Country","Region","Clade")
